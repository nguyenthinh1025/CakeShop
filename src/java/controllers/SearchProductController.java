...
public class SearchProductController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        try {
            String searchName = request.getParameter("txtSeachName");
            String searchCata = request.getParameter("txtSeachCatagory");
            ProductDAO dao = new ProductDAO();
            CatagoryDAO daoC = new CatagoryDAO();
            List<CatagoryDTO> listCatagory = daoC.getListCatagory();
            if (listCatagory != null) {
                request.setAttribute("LIST_CATA", listCatagory);
                url = SUCCESS;
            }
            List<ProductDTO> list = dao.getListProduct(searchName, searchCata);
            if (list != null) {
                url = SUCCESS;
                request.setAttribute("LIST_PRODUCT", list);
            } else {
                request.setAttribute("ERROR", "KHONG TIM THAY SAN PHAM!!!!");
            }
        } catch (Exception e) {
        } finally {
